...
.PHONY: \
	...
	restart-ui \
	dev-api dev-ingest dev-communications dev-ui dev-worker reconcile reconcile-llm-budgets run-llm-budget-alerts seed-event-deliveries seed-costs seed-agent-overrides migrate merge-heads rollback makemigrations test-api test-ui lint-ui check-ui coverage check-api check-migrations check-monitoring fix-api test check fix \
