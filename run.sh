...
# runs migrations, then starts db/redis/api/worker/communications/credential-gateway/ui
...
step "Building and starting api, worker, communications, credential-gateway, ui"
${COMPOSE} up -d --build api worker communications credential-gateway ui
...
box_line "Credential gateway -> $(printf '\033[1mhttp://localhost:%s\033[0m' "${GATEWAY_PORT:-8003}")"
green "+------------------------------------------------------+"
printf '\n'
