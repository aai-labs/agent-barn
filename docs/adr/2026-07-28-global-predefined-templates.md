...
- Editing a platform predefined template creates the first organization snapshot at Organization v1, independent of the platform version. The row stores its platform source ID and baseline version. Later organization edits and explicit platform updates create subsequent organization versions; a platform update clones the complete platform snapshot and does not move existing Agent pins. The seeder only ever refreshes the platform v1 in place; org forks are never clobbered.
