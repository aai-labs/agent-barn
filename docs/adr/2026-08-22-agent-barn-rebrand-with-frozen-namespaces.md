...
- `Config.ingest_base_url` now targets the in-namespace `agentbarn-api` Service. An Agent receives that value only when its runtime Secret is generated at start, so the ingest cutover requires a sequential lifecycle restart of every running Agent before retiring `agentfarm-api`. `api.scripts.rebuild_running_agents` is the operator-only, dry-run-by-default command for this and other maintenance rebuilds; it records lifecycle events with a `SYSTEM` actor and is deliberately not exposed through HTTP.
