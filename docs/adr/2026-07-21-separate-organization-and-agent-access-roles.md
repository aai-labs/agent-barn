...
Agent Farm uses two role families because organization governance and authority over one Agent answer different questions. Every Membership persists one fixed Organization Role enum value—Owner, Admin, or Member—whose immutable Permission mapping is defined in code, while each explicit Agent Access relationship carries a database-backed Agent Access Role whose permissions govern that Agent aggregate. Organization Role permissions do not grant assigned-Agent operations.
...
The platform Permission vocabulary remains shared, but fixed Organization Role grants are code-owned while Agent Access Role grants are database-backed. Tokens do not embed grants; the current Membership role and Agent access state are resolved on each request. Agent access-management UI is deferred to AF-217, and event/audit infrastructure remains deferred to AF-218 through AF-221.
