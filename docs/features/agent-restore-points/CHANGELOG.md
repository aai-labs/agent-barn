...
- Changed (review): reclamation falls back to the resource's own name when the id label is
  absent. Label-only matching could never reach anything created before the label shipped — the
  exact leak this ticket targets — and re-warned about each one every ten minutes with nothing
  able to clear it. The names are generated by the same module that parses them
  (`restore-point-<uuid>`, `rp-cap-<uuid>`, `rp-res-<uuid>-<suffix>`), so the fallback is a second
  exact identifier, not a guess, and a resource matching neither route is still never deleted.
  The manual cleanup this previously required is no longer needed.
...
- Fixed (review): `resolved` counted a terminal row claimed only for the replay it owes. Such a
  row returns straight back out of resolution, so the summary reported work that did not happen;
  the reconciler now branches before calling rather than after.
- Changed (review): `startingDeadlineSeconds` on the CronJob. Left unset, the controller counts
  every missed window since the last run and stops scheduling permanently past 100 of them —
  about 17 hours of controller downtime at a 10-minute schedule, after which nothing is reclaimed
  until somebody notices. The other two CronJobs in the chart share this gap and are left for a
  separate change.
