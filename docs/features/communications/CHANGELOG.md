...
### 2026-09-08 — Slack mentions from other agents — PR pending

- Delivered: A Slack Connection can opt in to **Mentions from other agents** (`accept_agent_mentions`, default off). With it enabled, a message written by another bot, including another Agent Barn Agent, is admitted when it @mentions this Agent; without a mention it is journaled as `mention_required`, and the Agent's own messages stay `bot_ignored`. Bot messages never inherit thread ownership under the `start_only` thread policy, so two Agents cannot keep each other awake on unmentioned replies, and admission fails closed when ingress did not capture the bot identity. The default preserves today's behaviour: every bot-authored message is refused as `bot_ignored`.
- Notes: This is the seam for multi-agent pipelines in one channel, where one Agent opens a thread and @mentions the next. The Slack manifest already subscribes to `message.channels`, so no Slack app change is needed; operators flip the setting on the Connection. The setting is schema-driven and appears in the Connection form without UI changes. No persistence or migration change: it is a new optional key in the plugin-owned settings document.
