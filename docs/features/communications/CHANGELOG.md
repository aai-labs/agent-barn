...
- Changed: Teams app packages set `supportsFiles: true`, and personal-chat file download activities reach the runtime like other inbound attachments. Teams still does not advertise full attachment support: arbitrary outbound files require either the asynchronous personal-chat consent-card callback flow or separately consented Microsoft Graph/SharePoint access for channel and group-chat files. A Teams reply that contains produced files therefore names the files it could not attach instead of leaking `MEDIA:` control text or silently claiming success.
