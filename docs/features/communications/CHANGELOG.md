...
- Next: add Agent Barn Chat as another adapter at the Platform Plugin seam, then evaluate iMessage transport constraints independently of Agent runtimes. Email supports inbound and reply, but not agent-initiated outbound; initiated delivery on Discord, Telegram, and Teams remains unsupported through the Communications Gateway until each plugin supplies and tests target resolution and policy enforcement; native Discord and Telegram deliver scheduled results to their origin or configured home.
- Blockers: none.

## Changes

### 2026-09-21 — AF-321 — Never lose a triggered event — PR pending

- Delivered: A webhook event that arrives while its agent is stopped now waits and runs once the agent starts. Chat messages are still dropped as `UNAVAILABLE`. Only events queue; each delivery kind's policy (`queue_while_stopped`) decides.
- Delivered: Each webhook keeps a limited number of waiting events (`COMMUNICATIONS_EVENT_BACKLOG_CAP`, default 100). Past it, the oldest is dead-lettered with the code `BACKLOG_CAP_EXCEEDED`.
- Changed: A failed run and a lost run are no longer treated the same. For an event, a failure the runtime reported is final on the first attempt (`COMMUNICATIONS_EVENT_MAX_ATTEMPTS_AFTER_FAILURE`, default 1). A claim whose lease ran out is tried again, up to 2 attempts (`COMMUNICATIONS_EVENT_MAX_ATTEMPTS_AFTER_LEASE_EXPIRY`). Chat keeps 5 attempts for both. Before this, only failures classified as retryable were ever retried; an unrecognised error was already final.
- Delivered: An agent runs at most 3 events at once (`COMMUNICATIONS_MAX_IN_FLIGHT_EVENT_RUNS_PER_AGENT`). At the cap the claim skips events and leaves them `PENDING` with no attempt used. Chat is never counted or held back. `GET /agents/{agent_id}/connection-runs` reports `in_flight`, `max_in_flight` and `queued` for event runs. The API also wakes the pod when a run finishes, so the next event starts at once.
- Changed: The pod adapter waits for its runtime to answer (`/health` on Hermes, `/startup` on OpenClaw) before it claims anything. Without this, every queued event would fail when its agent starts, and a failed event is final. The change reaches an agent the next time it starts.
- Changed: When the pod claims an event, the webhook plugin puts one fixed line above the caller's prompt ("Triggered by a webhook event…"). The stored prompt and the calls list are unchanged. Every agent's `AGENTS.md` gains a short "Triggered Runs" section that explains the line and says to look in the destination before creating something a second time.
- Changed: Two fixed messages, the lease-expired one and the backlog one, are now shown as written instead of being redacted.
- Follow-up: Deleting an agent or removing a webhook already ends its waiting events as `CANCELLED` (`CONNECTION_RETIRED`), so nothing stays `PENDING`. The ticket text says `DEAD_LETTERED`. Old deliveries are still never pruned, and pruning must ship together with replay protection. The four new settings are not in the Helm chart yet.
