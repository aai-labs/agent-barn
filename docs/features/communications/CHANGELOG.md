...
### 2026-09-16 — Runtime-neutral file attachments — PR pending

- Delivered: Slack, Discord, and Telegram now normalize provider file metadata, lazily download inbound files into the Agent workspace before a turn, and upload runtime-produced files on replies. Slack `file_share` events are admitted instead of dropping the file and its accompanying text; attachment-only messages on every supported inbound transport become a concrete workspace-path prompt instead of a blank turn. Discord replies are split at its 2,000-character provider boundary with a deterministic nonce per chunk.
- Delivered: the shared runtime adapter recognizes line-start `MEDIA:<absolute /workspace path>` control lines already required by the built-in artifact instructions. It removes those lines from visible text, uploads the referenced bytes idempotently over the Agent-authenticated Communications protocol, and queues Agent-scoped attachment references. Replies accept at most ten files totaling at most 20 MiB. Stored metadata, not runtime-supplied filename or media-type claims, is authoritative when a reply is queued; successful delivery deletes bytes atomically, while dead-lettered delivery retains them for operator retry.
- Changed: Teams app packages set `supportsFiles: true`, and personal-chat file download activities reach the runtime like other inbound attachments. Produced personal-chat files are delivered through Teams' asynchronous consent-card callback flow; their bytes remain durable until the recipient accepts and Communications uploads to the supplied OneDrive URL. Channel and group-chat uploads still require separately consented Microsoft Graph/SharePoint access and are rejected rather than silently dropped.
- Changed: attachment content is stored in a dedicated table keyed by Agent and runtime idempotency key. Runtime upload/download routes reuse Communications protocol authentication; provider references and stored content are both resolved under the authenticated Agent and source Delivery boundary.
