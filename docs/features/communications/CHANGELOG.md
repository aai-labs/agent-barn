...
### 2026-09-08 — Opt-in admission of explicitly addressed bot messages on Slack — PR pending

- Delivered: A Slack Connection can opt in to **Mentions from other bots** (`accept_bot_mentions`, default off). With it enabled, a message written by any Slack bot, including another Agent Barn Agent, is admitted when it explicitly @mentions this Agent; without a mention it is journaled as `mention_required`. The Agent's own messages stay `bot_ignored`, bot messages never inherit thread ownership under the `start_only` thread policy, and admission fails closed both when ingress did not capture the bot identity and for legacy bot events that carry only `bot_id` (a different id space from the bot user id, so a self-authored legacy event cannot be recognised). The default preserves today's behaviour: every bot-authored message is refused as `bot_ignored`.
- Notes: This is admission only, not Agent collaboration. Security decision, made explicit in the setting's description: with the toggle on, any bot that shares a permitted channel can address the Agent, so the setting is off by default and meant for Agents working in controlled channels. Peer discovery for Agents, a known-peer trust boundary (same Organization and workspace), sender provenance passed to the recipient runtime, exclusive active thread ownership for `start_only`, and bounded context transfer are deliberately out of scope and tracked as follow-up design work. The Slack manifest already subscribes to `message.channels`, so no Slack app change is needed. The setting is schema-driven and appears in the Connection form without UI changes; no persistence or migration change.
