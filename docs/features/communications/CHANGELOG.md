...
- Delivered: generic Agent-owned Communication Connections, multiple same-platform connections, the code-owned plugin catalogue, shipped Slack/Teams/Telegram/Discord plugins, credential encryption/fingerprints, safe read projections, Agent-permission authorization, optimistic concurrency, retirement, durable delivery workers, supervised provider ingress with replica-safe leases, authenticated Teams webhooks, and runtime protocol adapters.
