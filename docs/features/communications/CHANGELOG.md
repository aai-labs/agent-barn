...
- Delivered: Hermes and OpenClaw can durably submit scheduled final responses to Communications. Jobs created in a conversation return to that Connection, channel, and thread; jobs created at startup use the Agent's single configured default. Shared silence markers suppress empty scheduled updates.
...
- Changed: Initiated submissions persist canonical message and delivery state atomically under an Agent-scoped idempotency key, use a distinct `initiated_queued` journal stage, and retry scheduled completions from a runtime-local SQLite spool. The Connection editor configures the one default target enforced by PostgreSQL.
