...
- Reads require Agent visibility; mutations require Agent update and secret-management authority. Invocation content requires activity-read authority.
- The signing secret is encrypted at rest and returned only when created or rotated.
- Public ingress verifies HMAC-SHA256 over the exact raw request body before validating the versioned payload.
...
- A UI retry is allowed only from `DISPATCH_FAILED`; it increments the dispatch generation and resubmits the same invocation.
...
2. An external system posts a versioned JSON body to the public URL and signs the raw bytes.
