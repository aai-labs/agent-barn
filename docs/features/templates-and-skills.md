...
- Content edits go through a `skill_draft` row: at most one per lineage (an `skill_draft_file` set), mirroring `PlatformTemplateDraft`. `POST /{skill_id}/draft` gets-or-creates it, seeded from the latest published version by default or, with `?source_version=N`, from an older one (this is how rollback works — it never mutates history, it just seeds a draft for review). `PATCH /{skill_id}/draft` replaces its files; `DELETE /{skill_id}/draft` discards it; `POST /{skill_id}/draft/publish` turns it into the next immutable `skill_version`, records `restored_from_version` when the draft was seeded from an older one, and clears the draft slot. All draft endpoints require `skill.manage` and are rejected for built-ins. `PATCH /{skill_id}` itself is metadata-only (name, description, required providers) and is never draft-gated or versioned, since it isn't part of the mounted content.
