...
- Content edits go through a `skill_draft` row: at most one per lineage (an `skill_draft_file` set), mirroring `PlatformTemplateDraft`. `POST /{skill_id}/draft` gets-or-creates it, seeded from the latest published version. `PATCH /{skill_id}/draft` replaces its files; `DELETE /{skill_id}/draft` discards it; `POST /{skill_id}/draft/publish` turns it into the next immutable `skill_version` and clears the draft slot. All draft endpoints require `skill.manage` and are rejected for built-ins. `PATCH /{skill_id}` itself is metadata-only (name, description, required providers) and is never draft-gated or versioned, since it isn't part of the mounted content.
...
- `SkillVersionRead` carries `is_pinned_by_agent`, so the UI can disable the per-version Delete button and show a "Pinned by agent" badge for versions in use.
- `SkillDetailRead` carries `is_assigned_to_agent`, indicating whether any non-soft-deleted agent currently has the skill assigned (used by the fork flow).
