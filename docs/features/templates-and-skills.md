...
- Editing a platform predefined template creates the first organization snapshot at **Org v1**, regardless of the platform version. The row stores the original platform source ID, the current platform baseline ID, and the denormalized baseline version. Later organization edits publish Org v2, v3, and so on. Organization Template reads expose `platform_update_available` when the stored baseline version is behind the latest published platform version, and the Org Template UI labels the row as an Organization fork rather than a Built-in template. The seeder only bootstraps missing platform lineages; org forks are never clobbered.
