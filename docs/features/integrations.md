...
A provider whose Integration Plugin declares `EgressMode.GATEWAY_PROXY` is routed through the credential gateway whenever `CREDENTIAL_GATEWAY_ENABLED` is true. Ordinary HTTP profiles keep aai-cli's existing `auth_type = "bearer_token"`, point their existing `base_url` or `site_url` override at `/p/<provider>`, and use the existing `token_env` field to load `AF_GATEWAY_TOKEN_<PROVIDER>`. Zoho CalDAV keeps its existing `app_password` transport and carries the Gateway Token as the Basic-auth password because that transport does not use aai-cli's shared HTTP client. In both cases the token authenticates only the pod-to-gateway hop; the gateway replaces it with the provider's real authorization before forwarding. No aai-cli gateway-specific behavior is required.

All shipped aai-cli providers support this mode: GitHub, Jira, Confluence, Bitbucket, Zoho Mail REST, Zoho Calendar CalDAV, and Pipedrive. Zoho Mail's renewable OAuth credential stays in the gateway, which exchanges and caches a short-lived access token there. Credential-owned Jira, Confluence, and CalDAV upstream URLs are restricted to their provider hosts before forwarding. Gateway-routed providers are excluded from the aai-cli secret store, so their real credentials are in neither the pod Secret nor `aai-secrets.enc.json`. The plugin's `egress_mode` is the single provider-level decision, so adding a provider does not require a deployment allowlist change. Helm and new local environments enable the gateway globally; setting `CREDENTIAL_GATEWAY_ENABLED=false` rolls all providers back to direct materialization.
