...
Read before changing agent creation, Agent Access Roles or assignments, lifecycle, runtime or platform selection, template pinning, model selection, skill assignment, credentials, logs, health, or Kubernetes resources.
...
- Organization Owner/Admin and superuser in explicit Organization context have implicit Agent Owner authority over every Agent. An Organization Member requires explicit Agent Access carrying one Agent Access Role; inaccessible and cross-Organization Agents are concealed with 404.
- The locked Agent Viewer role grants read, activity, and cost access; Agent Editor adds configuration, lifecycle, Skill assignment, and credential management; Agent Owner adds deletion and access management.
