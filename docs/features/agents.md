...
- The API rejects direct configuration updates while an Agent is running, but running Agent read DTOs still expose the caller's configuration and secret permissions so the canonical UI can offer `Apply & Restart`: it stops the Agent, applies the section update, and starts it again. For stopped Agents it uses `Apply`, persists the update, and leaves the Agent stopped until the user starts it from the Agent detail page. Template Override drafts remain private until publish; a published version is applied only after explicit selection, using `Apply` while stopped or `Apply & Restart` while running.
