...
`POST /agents/{agent_id}/memory/shared-facts` promotes explicit, operator-supplied content into one or more destination Agents' pools, written as a conclusion on each destination's own self-model because that is where its recall looks. Because Honcho stores no metadata on a conclusion, the origin is tracked in `shared_memory_fact` and joined back on read, so a shared memory reads as "Shared by <agent>" rather than as something the Agent concluded about itself. `POST /agents/{agent_id}/memory/carry-over` copies a source Agent's whole pool memory into other Agents' pools, bounded to a rescue-sized batch. Both paths remain a deliberate cross-pool copy: distinct pools are separate Honcho workspaces (`workspace_name` participates in nearly every composite foreign key, so isolation is a schema property), and copying into the destination is the only mechanism that crosses that boundary — sharing *within* a group is automatic through the shared workspace and needs neither path. Both require the source and every destination to be in a group (409 otherwise). The source Agent of a shared fact is audit context only, so it needs `agent.read`; carry-over reads the source's memory, so it needs `agent.memory.read`; every destination needs `agent.memory.manage`, since the write changes what that pool knows. A destination must currently be an active (non-deleted) Agent.
