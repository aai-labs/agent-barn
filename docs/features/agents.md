...
Restore Points are surfaced on the canonical configuration page as their own section, gated on `activity.read` to read and `agent.lifecycle.manage` to capture, restore, or delete. The list reconciles on read, so the client polls it while any entry is non-terminal and stops when none is; it pages, because system-created and failed entries occupy the list without counting against the per-Agent cap and each entry holds a volume until it is deleted. Capture and restore are offered as disabled controls carrying their reason — Agent running, an operation already in flight, or the cap reached — rather than as requests the API refuses.

Restoring may optionally replay the recorded configuration. The replay is opt-in, off by default, and requires `agent.update` in addition to the lifecycle permission that authorizes the volume operation. It is applied as a single template selection carrying the recorded scope, template version, skill pins and runtime settings, so a pin that is no longer valid is refused by the same validation as any other configuration change, and the whole recorded configuration either lands or does not — there is no half-applied state to report. It remains a separate operation from the volume restore, which is already running by the time the replay is attempted and stands regardless of its outcome.
