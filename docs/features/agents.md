...
Each Restore Point also records the Agent's pins at capture time — template pin, model, command approval, verbose mode, and skill version pins — in a versioned, display-oriented configuration manifest. The manifest never holds credentials. It records the template pin's scope (platform, organization, or Agent-owned override) rather than the display pin type, because an Organization fork shares its platform lineage's template key and restarts its own version numbering, so scope is what separates two pins that otherwise read alike. Manifest version 1 predates the template and skill pins; a Restore Point carrying one reports those fields as unrecorded, which is not the same as recorded-and-empty, and cannot be replayed.

Restore Points are surfaced on the canonical configuration page as their own section, gated on `activity.read` to read and `agent.lifecycle.manage` to capture, restore, or delete. The list reconciles on read, so the client polls while any entry is non-terminal or owes configuration replay, and stops once both are resolved; it pages, because system-created and failed entries occupy the list without counting against the per-Agent cap and each entry holds a volume until it is deleted. Capture and restore are offered as disabled controls carrying their reason — Agent running, an operation already in flight, or the cap reached — rather than as requests the API refuses.

Restoring may optionally replay the recorded configuration. The replay is opt-in, off by default, and both permissions are settled when the restore is requested: `agent.update` for the configuration alongside the lifecycle permission for the volume. The recorded configuration is checked before the Job starts, so one that can no longer be applied refuses the restore outright rather than costing the Agent its files first. The intent is then stored on the restore point and the configuration is written only after the Job confirms the volume is back — a restore that fails leaves the configuration untouched, and no browser has to stay open for the replay to happen. The intent is swept separately from the row's status, so work interrupted between the two survives; completion is committed atomically with the configuration and its audit events under the Agent's lifecycle lock, so two readers cannot both apply it and it cannot land beside a start or a delete; pending replay also blocks lifecycle operations until it is resolved; and it is attributed to the Member who asked for the restore, not to whoever captured the restore point. The selection is rebuilt on the server from the stored manifest, so what was checked before the Job is what is written after it, and it goes through the same validation and the same write as any other configuration change. If it cannot be applied by then, the volume restore still stands and the restore point records why the configuration did not follow.
