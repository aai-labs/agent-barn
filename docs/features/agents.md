...
Stop snapshots logs before removing active runtime resources and marking the agent stopped. A successful transition to `STOPPED` emits `agent.stopped`; its email handler notifies the Agent Creator and users with Agent Owner access, de-duplicated by email. Delete removes resources, soft-deletes the row, and preserves the record for history and cost attribution. Deletion also clears the Slack or Discord bot token hash, releasing the token for reuse by another Agent on the same platform.
