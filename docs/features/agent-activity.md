...
- A wake is `USER` when an inbound `agent_chat_message` for that Agent falls within `[started_at - USER_LEAD_SECONDS, ended_at]`, and `BACKGROUND` otherwise. Only direction and occurrence time are read; message content never is. Agent Webhook invocations are dispatched to the runtime directly and write no chat message, so webhook-triggered work is always `BACKGROUND`. If machine triggers ever start writing inbound chat messages, this predicate must exclude them.
