...
- **Endpoint.** `POST /organizations/{org}/memory-groups/{source_group_id}/shared-items` `{ memory_id, target_group_ids }`, gated on `memory_group.manage`, org-scoped. Reads the conclusion from the source pool, copies it into each target pool (onto the neutral `owner` peer, so it is recalled pool-wide and attributed to no single member), and reports a per-target result. Rejects a group sharing with itself (400) and a missing memory (404); 409 when Honcho is off.
- **Provenance.** New `shared_pool_memory_fact` table (in the agents domain, so `list_memory` joins it without a cross-domain cycle) records the origin group. The memory read returns `sharedFromGroupId`; the client resolves the name and badges the item "Shared from <group>". Deleting the source group nulls the id (badge falls back to "Shared in"); deleting the target group drops the row via CASCADE. Forget/correct carry the pool provenance the same way they already did the Agent-level one.
- **UI.** The memory tab's per-item share action now targets **groups** (a picker of the org's other groups), shown to a group manager when the Agent is in a group. The old Agent-targeted share dialog and its `shared-facts`/`carry-over` endpoints stay in the codebase but are no longer wired into this view.
- **Coverage.** Service tests for the copy + per-target results + origin recording and the 400/404/409 guards; read-path tests that `sharedFromGroupId` is attached and that forget/correct carry the pool provenance. `check-api`, `check-migrations`, `check-ui`, `lint-ui` clean; single migration head.
- **Cross-group recall verified live** (2026-09-22, Honcho v3.1.0 + LiteLLM). Replayed the exact destination write (`share_fact` onto the pool's `owner` peer) via our own client, then queried the workspace-level dialectic (`POST /v3/workspaces/{ws}/chat`) both ways the runtimes call it — the OpenClaw plugin's `{query, target:"owner"}` and the Hermes patch's `{…, stream, reasoning_level}`. Both recalled the shared fact ("March 3rd, 2026") from a pool that also held unrelated member memory, confirming the owner-peer placement surfaces pool-wide on both runtimes.
