...
- **Coverage.** Service tests for the copy + per-target results + origin recording and the 400/404/409 guards; read-path tests that `sharedFromGroupId` is attached and that forget/correct carry the pool provenance. `check-api`, `check-migrations`, `check-ui`, `lint-ui` clean; single migration head. Cross-group recall on both runtimes is pending the same live re-verify as the groups work.
