...
A Gateway Token is a credential to Agent Barn, not to a provider. Agent start issues one per gateway-served `(Agent, provider)` pair and writes it to the pod Secret as `AF_GATEWAY_TOKEN_<PROVIDER>`; agent stop revokes them. Each Integration Plugin's `EgressMode` is the provider-level source of truth, and one global `CREDENTIAL_GATEWAY_ENABLED` switch provides operational rollback without a duplicate provider list. All aai-cli providers support gateway proxying, and the gateway is enabled by default in Helm and new local environments.
