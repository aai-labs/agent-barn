...
Both Hermes and OpenClaw consume the same versioned Communications protocol. A sidecar-style runtime adapter opens an authenticated, outbound Server-Sent Events control stream to Communications. A `delivery_available` wakeup makes the adapter claim durable inbound Communication Deliveries, invoke the runtime's local chat-completions endpoint with a Connection-scoped session key, submit the reply against the source delivery, and complete the delivery. Runtimes never receive provider tokens and contain no Slack, Telegram, or Discord transport configuration.
