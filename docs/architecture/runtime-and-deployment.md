...
Cancellation is durable before it is signalled. A cancelled source cannot enqueue a reply, and cancellation wins over a late successful runtime completion. A processing Delivery exposes `cancel_requested_at` immediately while its status remains `PROCESSING`, allowing Web Chat to stop showing it as awaiting a reply. Neither pinned runtime exposes a proven abort handle for its OpenAI-style chat-completions request, so cancellation is currently soft: local computation may finish, but its result is suppressed and the Delivery is terminally `CANCELLED`. Agent pods expose no Communications control listener or admin Service port.
