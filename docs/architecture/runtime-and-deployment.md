...
Inbound provider files are represented by metadata in the durable delivery envelope and downloaded lazily by the runtime adapter into `/workspace/.agentbarn/inbox/<delivery-id>/` before the turn starts. Runtime-produced files use a line-start `MEDIA:<absolute /workspace path>` control line. The adapter removes those lines, uploads at most ten files totaling at most 20 MiB through the authenticated Communications protocol, and submits only Agent-scoped durable references with the reply. Communications owns the bytes until provider delivery succeeds; dead-lettered replies retain them for operator retry. Slack, Discord, and Telegram support inbound download and outbound upload. Teams supports inbound personal-chat files through its `supportsFiles` manifest contract, but outbound files require an asynchronous personal-chat consent-card flow or separately consented Microsoft Graph/SharePoint access; until that exists, Teams sends the reply text plus an explicit list of files it could not attach and does not advertise full attachment support.
