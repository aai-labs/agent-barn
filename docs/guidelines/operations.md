...
- **Local runtime-owned Teams:** the API cannot resolve k3d cluster DNS or reach ClusterIPs, so the relay target comes from `TEAMS_RUNTIME_WEBHOOK_URL`. `make dev-api` defaults it to `http://localhost:3978/api/messages`; `compose.yml` instead defaults it to `http://host.docker.internal:3978/api/messages`. Run `make forward-teams AGENT=<agent-uuid>` to publish that Agent's port 3978 on the host, and re-run it after the pod restarts. Override the variable only when the port-forward uses a different host or port.
