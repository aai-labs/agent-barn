...
- **Each environment has its own pair, and all three values must differ**: `STAGING_AGENT_EMAIL_DOMAIN`/`STAGING_EMAIL_INBOUND_SECRET` for staging, `AGENT_EMAIL_DOMAIN`/`EMAIL_INBOUND_SECRET` for the k3s `main` namespace, and `PUBLIC_AGENT_EMAIL_DOMAIN`/`PUBLIC_EMAIL_INBOUND_SECRET` for the hosted public cluster. The publish fails when the selected secret is empty: the API rejects a blank configured secret outright, so an unset value bounces every inbound message with `401` rather than degrading.
