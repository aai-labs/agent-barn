...
CONTRACT_PAYLOAD = (
    Path(__file__).resolve().parents[3] / "workers" / "email-inbound" / "contract" / "inbound-payload.json"
)
...
def test_the_payload_the_worker_actually_produces_is_understood_by_the_plugin() -> None:
    """Pins the plugin against a recorded payload from the Cloudflare Worker.

    `contract/inbound-payload.json` is generated by running the Worker's own
    `buildPayload` over `contract/inbound.eml`, so this fails if either side stops
    agreeing about the wire format. Every key asserted here is one `send()` later
    reads back out of `provider_metadata`.
    """
    plugin = _plugin()
    contract = json.loads(CONTRACT_PAYLOAD.read_text(encoding="utf-8"))

    [envelope] = plugin.normalize_inbound(_settings(plugin), contract)

    assert envelope.provider_message_id == contract["message_id"]
    assert envelope.location.id == contract["from"]
    assert envelope.location.thread_id == contract["references"][0]
    assert envelope.sender.display_name == contract["from_name"]
    assert envelope.provider_metadata["recipient"] == contract["to"]
    assert envelope.provider_metadata["subject"] == contract["subject"]
    assert envelope.provider_metadata["message_id"] == contract["message_id"]
    assert envelope.provider_metadata["references"] == " ".join(contract["references"])
...
def test_the_persisted_message_is_only_what_the_sender_wrote() -> None:
    ...
    assert envelope.text == "What does the team plan cost?"
...
def test_the_agent_is_given_the_sender_and_subject_the_adapter_does_not_pass_through() -> None:
    ...
    assert "What does the team plan cost?" in prompt
...
def test_the_agent_is_told_the_message_is_addressed_to_it_and_that_its_reply_is_sent() -> None:
    ...
    assert prompt.lower().index("reply") < prompt.index("What does the team plan cost?")
