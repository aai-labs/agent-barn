...
_JIRA_SCOPED = JiraContent(
    site_url="https://acme.atlassian.net", api_token="scoped-tok", use_scoped_token=True
)
...
_JIRA_CLOUD_ID_MOD = (
    "api.infrastructure.integration_validators.jira.get_atlassian_cloud_id"
)


def test_jira_scoped_token_valid_returns_identity():
    """Scoped token: cloud_id resolved, Bearer call succeeds — valid with project count."""
    projects = [{"key": "PROJ"}, {"key": "DEMO"}, {"key": "TEST"}]
    with (
        patch(_JIRA_CLOUD_ID_MOD, return_value=("cloud-abc", None)),
        patch(_JIRA_MOD, return_value=_resp(projects)),
    ):
...
def test_jira_scoped_token_cloud_id_fetch_fails_hard_fails():
    ...
    with patch(
        _JIRA_CLOUD_ID_MOD, return_value=(None, "Network error fetching cloud ID")
    ):
        result = validate_jira(_JIRA_SCOPED)
...
def test_jira_scoped_token_bearer_401_returns_error():
    """Scoped token: cloud_id OK but Bearer call returns 401 — invalid token."""
    with (
        patch(_JIRA_CLOUD_ID_MOD, return_value=("cloud-abc", None)),
    ...
    assert result.valid is False
    assert (
        "invalid" in (result.error or "").lower()
        or "token" in (result.error or "").lower()
    )
...
_CONF_CLOUD_ID_MOD = (
    "api.infrastructure.integration_validators.confluence.get_atlassian_cloud_id"
)


def test_confluence_scoped_token_valid_returns_identity():
    """Scoped token: cloud_id resolved, Bearer call succeeds — valid with space count."""
...
def test_confluence_scoped_token_cloud_id_fetch_fails_hard_fails():
    """Scoped token: cloud_id resolution fails — validation must hard fail immediately."""
    with patch(
        _CONF_CLOUD_ID_MOD, return_value=(None, "Network error fetching cloud ID")
    ):
        result = validate_confluence(_CONFLUENCE_SCOPED)
...
def test_confluence_scoped_token_bearer_401_returns_error():
    """Scoped token: cloud_id OK but Bearer call returns 401 — invalid token."""
    with (
        patch(_CONF_CLOUD_ID_MOD, return_value=("cloud-abc", None)),
