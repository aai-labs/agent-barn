...
from api.domains.memory_groups.models import MemoryGroup, MemoryGroupCreate, MemoryGroupUpdate
...
def _service(*, honcho_enabled: bool = True):
    ...
    return service, repository, permission_policy, honcho, agent_service


def test_create_group_requires_the_manage_permission_and_persists():
    org_id = uuid7()
    service, repository, permission_policy, _honcho, _agents = _service()
...
def test_add_agent_assigns_the_group_via_agent_service():
    ...
    service, repository, _pp, _honcho, agent_service = _service()
...
def test_delete_group_deliberately_purges_the_shared_pool():
    ...
    org_id = uuid7()
    service, repository, _pp, honcho, _agents = _service(honcho_enabled=True)
    group = MemoryGroup(id=uuid7(), organization_id=org_id, name="Research")
    repository.get_by_id_and_org.return_value = group
...
def test_delete_group_skips_pool_purge_when_memory_backend_is_off():
    org_id = uuid7()
    service, repository, _pp, honcho, _agents = _service(honcho_enabled=False)
    group = MemoryGroup(id=uuid7(), organization_id=org_id, name="Research")
    repository.get_by_id_and_org.return_value = group
...
def test_rename_group_persists_the_new_name():
    org_id = uuid7()
    service, repository, _pp, _honcho, _agents = _service()
...
def test_unauthorized_caller_cannot_manage_groups():
    org_id = uuid7()
    service, _repository, permission_policy, _honcho, _agents = _service()
