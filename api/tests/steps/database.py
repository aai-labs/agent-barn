...
def database_is_clean():
    def step(context):
        ...
        with delegate.engine.connect() as conn:
            conn.execute(
                text(
                    "TRUNCATE security_audit_record, gateway_audit_event, event_delivery, event_outbox_message, communication_operation_journal, skill, communication_delivery, agent_chat_message, communication_connection, tool_call, agent_template, platform_template_draft_skill, platform_template_draft, platform_template_skill, platform_template, agent CASCADE"
                )
            )
            conn.commit()
