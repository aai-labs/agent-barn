...
def test_start_agent_injects_triggered_runs_policy_into_agents_md_openclaw():
    with given([*_GIVEN, there_is_an_agent()]) as context:
        client: TestClient = context.client
        k8s: MagicMock = context.injector.get(KubernetesClient)
        ...
        with then("AGENTS.md tells the agent how to treat a run another system started"):
            assert_that(response.status_code, equal_to(status.HTTP_200_OK))
            config_map = k8s.create_config_map.call_args.args[1]
            agents_md = config_map.data["AGENTS.md"]
            assert_that(agents_md, contains_string("## Triggered Runs"))
            assert_that(agents_md, contains_string("Triggered by a webhook event"))


def test_start_agent_injects_triggered_runs_policy_into_agents_md_hermes():
    with given([*_GIVEN_WITH_HERMES_IMAGE, there_is_an_agent(agent_type=AgentType.HERMES)]) as context:
        client: TestClient = context.client
        k8s: MagicMock = context.injector.get(KubernetesClient)
