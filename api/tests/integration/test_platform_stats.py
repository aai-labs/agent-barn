    ...
    repository.upsert_messages(
        [
            AgentChatMessage(
                ...
                direction=direction,
                sender_id="U1" if direction == MessageDirection.INBOUND else None,
                content="hello",
...
def test_message_stats_buckets_the_series_by_utc_day():
    ...
    with given(
        ...
        agent_id = context.agent.id
        _seed_message(context, agent_id=agent_id, direction=MessageDirection.INBOUND, occurred_at=day_one)
        _seed_message(context, agent_id=agent_id, direction=MessageDirection.OUTBOUND, occurred_at=day_two)
    ...
    with given(
        [
            *_BASE_GIVEN,
            ...
            there_is_an_agent(name="Retired Agent", deleted=True),
