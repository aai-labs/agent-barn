...
def validate_bitbucket(content: BitbucketContent) -> IntegrationValidationResult:
    try:
        ...
        return IntegrationValidationResult(valid=False, error=f"Could not reach Bitbucket: {exc}")
...
def _handle_user_response(resp: httpx.Response, content: BitbucketContent) -> IntegrationValidationResult:
    if resp.status_code == 401:
        return IntegrationValidationResult(valid=False, error="Invalid API token or credentials")
...
def _handle_user_scope_denied(resp: httpx.Response, content: BitbucketContent) -> IntegrationValidationResult:
    ...
    missing: list[str] = []
    if "read:repository:bitbucket" not in granted and "write:repository:bitbucket" not in granted:
        missing.append("Repositories (read) scope missing")
    if "read:pullrequest:bitbucket" not in granted and "write:pullrequest:bitbucket" not in granted:
        missing.append("Pull requests: Read + Write — needed to read PRs and post review comments")
...
def _validate_scoped_token(content: BitbucketContent) -> IntegrationValidationResult:
    ...
    if resp.status_code == 401:
        return IntegrationValidationResult(valid=False, error="Invalid or expired access token")
...
def _validate_repo_scoped_tokens(content: BitbucketContent, headers: dict[str, str]) -> IntegrationValidationResult:
    ...
    for repo in content.repos:
        ...
        if resp.status_code == 401:
            return IntegrationValidationResult(valid=False, error="Invalid or expired access token")
        if resp.status_code == 403:
            failures.append(f"{content.workspace}/{repo}: missing the Repositories (read) scope")
            continue
        if resp.status_code not in (200, 404):
            failures.append(f"{content.workspace}/{repo}: unexpected status {resp.status_code}")
            continue
...
def _check_repo_read_scope(content: BitbucketContent, *, bearer: bool = True) -> list[str]:
    if not content.workspace:
        return []
