...
def upgrade() -> None:
    ...
    op.create_foreign_key('fk_agent_skill_skill_id', 'agent_skill', 'skill', ['skill_id'], ['id'], ondelete='CASCADE')
...
def downgrade() -> None:
    ...
    op.add_column('agent_skill', sa.Column('source', sa.VARCHAR(), nullable=False, server_default='aai_cli'))
    op.add_column('agent_skill', sa.Column('skill_name', sa.VARCHAR(length=255), nullable=False, server_default=''))
    op.add_column('agent_skill', sa.Column('skill_file_path', sa.VARCHAR(length=1024), nullable=False, server_default=''))
    op.add_column('agent_skill', sa.Column('skill_content', sa.TEXT(), nullable=False, server_default=''))
