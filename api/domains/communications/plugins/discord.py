...
class DiscordPlatformPlugin(PlatformPlugin):
    ...
    setup_hint = (
        ...
        "4. Add the token here\n"
        "Paste the Bot Token into this Connection and save it.\n\n"
        "5. Set access rules\n"
        "The bot must belong to each allowed server and view every allowed channel. Enable Developer Mode to copy guild, "
        "channel, user, and role IDs. Direct messages are Off by default; enable them only when needed. When Require "
        "@mention is on, people must mention the bot in server messages."
    )
