...
class HonchoUsageRepository:
    ...
    def record(self, events: list[HonchoUsageEvent]) -> int:
        ...
        statement = pg_insert(HonchoUsageEvent).values(rows).on_conflict_do_nothing(index_elements=["event_id"])
        with Session(self.delegate.engine) as session:
    ...
    def token_totals_by_workspace(self, start: datetime, end: datetime) -> list[WorkspaceTokenTotals]:
        statement = (
            select(
                HonchoUsageEvent.workspace_name,
                sa.func.sum(HonchoUsageEvent.input_tokens),
                sa.func.sum(HonchoUsageEvent.output_tokens),
            )
