...
export function PayloadHint() {
  return (
    <p className="mt-2 text-xs text-[var(--ink-4)]">
      Only <code>prompt</code> is required, up to 5,000 characters. Add an
      optional <code>event_id</code> to make caller retries safe: a repeated{" "}
      <code>event_id</code> returns the original invocation instead of
      triggering the Agent again.
    </p>
  );
}
...
export function AgentWebhookSettings({
  ...
  const selected = selectedId
    ? (webhooks.data?.find((webhook) => webhook.id === selectedId) ?? null)
    : null;
  ...
  async function addWebhook() {
    ...
    try {
      ...
      if (created.signingSecret)
        setReveal({ webhook: created, secret: created.signingSecret });
  ...
  return (
    <>
              ...
              !adding && (
                <div className="flex flex-col items-center gap-2 rounded-xl border border-dashed border-[var(--line-strong)] p-6 text-center">
                  ...
                  <p className="m-0 max-w-md text-sm text-[var(--ink-3)]">
                    Connect a CI job, Jira automation, or another machine. Each
                    webhook gets its own URL and signing secret.
                  </p>
                </div>
              )}
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs text-[var(--ink-4)]">
                      {PLATFORM_LABEL[webhook.deliveryPlatform]}
                    </span>
                  </div>
                  <div className="mt-1 truncate font-mono text-xs text-[var(--ink-4)]">
                    {webhook.webhookUrl}
                  </div>
                </div>
      ...
      <ConfirmationDialog
        ...
        description={
          <>
            Removing <strong>{retiring?.displayName}</strong> immediately
            invalidates its URL.
          </>
        }
...
function CreateWebhookForm({
  ...
  return (
    <div className="rounded-xl border border-[var(--line)] bg-[var(--bg-soft)] p-4">
      ...
      />
      <label
        htmlFor="webhook-platform"
        className="mb-1.5 mt-3 block text-sm font-medium text-[var(--ink)]"
      >
        Deliver results to
      </label>
      <Select
        value={platform}
        onValueChange={(value) =>
          onPlatformChange(value as WebhookDeliveryPlatform)
        }
        disabled={platformsLoading || platforms.length === 0}
      >
        <SelectTrigger id="webhook-platform" className="w-full">
          <SelectValue
            placeholder={
              platformsLoading ? "Loading channels…" : "Choose a channel"
            }
          />
        </SelectTrigger>
        <SelectContent>
          <SelectGroup>
            {platforms.map((item) => (
              <SelectItem key={item.key} value={item.key}>
                {deliveryPlatformOptionLabel(item)}
              </SelectItem>
            ))}
          </SelectGroup>
        </SelectContent>
      ...
      {!platformsLoading && platforms.length === 0 && (
        ...
        >
          ...
          <p className="m-0 text-xs leading-5">
            Configure and enable Slack, Discord, Telegram, or Teams with a
            default channel before adding a webhook.
          </p>
        </div>
      )}
      <p className="mt-2 text-xs text-[var(--ink-4)]">
      ...
      {error && (
        <p className="mt-2 text-xs text-[var(--err)]" role="alert">
          {error}
        </p>
      )}
      <div className="mt-3 flex justify-end gap-2">
        ...
        <button
          type="button"
          className="af-btn af-btn-primary"
          disabled={!displayName.trim() || !platform || pending}
          onClick={onSubmit}
        >
  ...
  return (
    ...
    >
      <div className="af-card p-6">
        <h2 className="m-0 text-lg font-semibold text-[var(--ink)]">
          Signing secret ready
        </h2>
        <p className="mt-1 text-sm text-[var(--ink-3)]">
          This secret cannot be viewed again. Store it in the calling system.
        </p>
        {[
          ["webhook-reveal-url", "Webhook URL", url],
          ["webhook-reveal-secret", "Signing secret", reveal.secret],
        ].map(([id, label, value]) => (
          <div className="mt-4" key={id}>
            ...
            <div className="flex gap-1.5">
              <input
                ...
                readOnly
                value={value}
                className="af-input flex-1 font-mono text-xs"
        ...
        <div className="mt-5 flex items-center justify-between">
          ...
          <CopyButton
            value={curlExample(url, reveal.secret)}
            label="Copy example request"
          />
        </div>
        <pre className="mt-1 overflow-x-auto rounded-lg bg-[var(--bg-soft)] p-3 text-xs text-[var(--ink-2)]">
          {curlExample(url, reveal.secret)}
        </pre>
        <PayloadHint />
