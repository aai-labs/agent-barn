...
export function TopNav({ onHire }: TopNavProps) {
  ...
  const navTabs = [
    ...
    // Audit log: a superuser's view is inherently all-orgs (a global route, no orgId,
    // where NULL-org auth events show), so route them there; an org admin gets the
    // org-scoped page whose org lives in the URL. Members can't see either.
    ...(canManageMembers || user.isSuperuser
      ? [
          {
            href: user.isSuperuser
